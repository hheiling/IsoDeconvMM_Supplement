...
Ultimately, the goal is to simulate exon set counts for each gene (or transcript cluster). We will simulate these counts in two ways. First, we will simulate negative binomial mean (and theta) parameters for exon sets for each gene cluster. Based on the assumed model given in "IsoDOT Detects Differential RNA-Isoform Expression/Usage with Respect to a Categorical or Continuous Covariate with High Sensitivity and Specificity" by Sun et al. (2015), the vector of negative binomial means for the $E$ exon sets for gene $g$ is $\mu_g = X_g \beta_g$, where $X_g$ is the effective length design matrix and $\beta_g$ is a vector $(\beta_{g1},...\beta_{gI})^T$ where $\beta_{gu}$ is proportional to the transcript abundance of the $u^{th}$ isoform. Second, we will simulate probability vectors for the exon sets and find counts using a multinomial distribution. See section "Isoform to Exon Set Parameters - Dirichlet Multinomial Model" for further details. In both cases, the effective length design matrix $X_g$ is needed.

In this section, the materials and procedures needed to create the effective length design matrix $X_g$ are described. In the next section "Simulate Counts for Exon Sets", the materials and procedures to simulate the necessary parameters for the simulations are described.
...
3. For each isoform, using the exon information from step 2, create string names for all exon sets with a single exon in the set: concatenate the cluster name, the gene ID (from nTE information) and exon number as described above. Store in a vector.
...
Now that we are armed with knowlegdge about the effective length design matrix, we can focus on calculating the necessary parameters.

# Simulate Counts for Exon Sets

This section outlines how the parameters and counts for all exon sets in each gene are to be calculated for each of the pure cell type reference samples.
...
2. Specify a Dirichlet distribution associated with all of the genes within a sample. For the purposes of this simulation, will fit a Dirichlet model based on some UCLA data and use this model as a 'realistic' Dirichlet model to use for simulation. See "Gene Samples and EffLen Design Matrix.Rmd" for further details about how genes were chosen for simulation and the fitting of the Dirichlet model.
...
5. Assign a constant dispersion parameter value across all genes. Specify the dispersion parameter such that the negative binomial variance is not much different than the variance for a poisson distribution. $Var(G) = \mu_g + \frac{1}{\theta}\mu_g^2$ with $\theta$ some large number, such as 100 (a range of 90 to 120).
...
## Isoform to Exon Set Parameters - Dirichlet Negative Binomial Model
...
$\rho_g$ = given a read is from gene $g$, vector of I probabilities that the read belongs to isoform $i$. Sampled from the Dirichlet dist in "Gene Cluster to Isoform Parameter" section. Vector of length I = number of isoforms in gene $g$.
...
## Isoform to Exon Set Parameters - Dirichlet Multinomial Model

### Available Materials and Known Models

From earlier, we have created the ExI effective length design matrix, $X_g = (x_{g1}, ..., x_{gI})$, for each gene cluster $g$. The columns of this design matrix correspond to the I isoforms for the gene cluster, and the rows correspond to the E exon sets for the gene cluster. We have also determined the gene cluster counts $T_g$ and the probability distribution for the isoforms $\rho_g$ within the gene cluster $g$. 

For each gene, let $\mu_g$ be a vector of exon set counts corresponding to all exon sets, $\mu_g = (\mu_{g1}, ..., \mu_{gE})^T$. This vector of counts is characterized by a multinomial distribution, described in the "Relationships" section.  
...
$T_{tot}$ = total read count in the sample

$\tau_g$ = proportion of counts in sample that belong to gene $g$

$Y_{g}$ = vector of $E$ exon set counts in gene $g$

$Y_{g(A)}$ = sum of counts for all other genes in sample outside of gene $g$

$\gamma_g$ = per-unit of effective length conditional probabilities that a read belongs to isoform $i$ given that it maps to gene $g$. Vector of length I.

$\tilde \ell_g$ = vector of total effective lengths of I isoforms. $\tilde \ell_{gi} = \sum_{e=1}^E X_{gei}$ for $i^{th}$ isoform. 

Relationships of interest:

$\sum_{i=1}^I \tilde \ell_i * \gamma_{gi} = 1 = \sum_{i=1}^I \rho_i$

$\gamma_g = \rho_g / \tilde \ell_g$

$\tau_g = T_g / T_{tot}$

$$\begin{bmatrix} Y_{g(A)} \\ Y_g  \end{bmatrix}
| \tau_g, \gamma_g \sim \text{Multinomial} \left(T_{tot}, 
\begin{bmatrix} 1 - \tau_g \\ \tau_g X \gamma_g \end{bmatrix} \right)
$$

For each gene, this can be re-written as:

$$Y_g | \gamma_g \sim \text{Multinomial}(T_g, s_g^\star X \gamma_g)$$

where $s_g^\star$ is the scaling factor that makes the sum of the vector $s_g^\star X \gamma_g$ equal 1.

### Details to Simulate Exon Set Counts

1. If we simulate isoform probability distribution parameters as described in the "Gene Cluster to Isoform Parameters" section, we can calculate the $\gamma_g$ parameters by first calculating $\tilde \ell$ ($\tilde \ell_{gi} = \sum_{e=1}^E X_{gei}$) and then calculating $\gamma_g = \rho_g / \tilde \ell_g$.

2. Calculate the vector $X \gamma_g$ and the scaling parameter $s_g^\star$ such that the sum of the vector $s_g^\star X \gamma_g$ equals 1. The new probability vector for the $E$ exon sets in gene $g$ is then $s_g^\star X \gamma_g$.

3. Calculate the counts $Y_g$ using the multinomial expression given above ($Y_g | \gamma_g \sim \text{Multinomial}(T_g, s_g^\star X \gamma_g)$)
...
C. Take the ratio of the proportions (proportion A / proportion B) and multiply this ratio by all of the gene counts (of the 100 differentially expressed genes) after fold change. 
...
**High Effect Size Option**

We will consider 3 cell types in this simulation. The genes specified to have differential isoform expression will have different Dirichlet distributions for each cell type. All of the genes selected for diffferential isoform usage will have 3 or more isoforms, so we will consider the following classes of Dirichlet distributions:

* 'uniform': All $\alpha$ parameters will be equal for each of the $I$ isoforms, resulting in probabilities close to $1/I$ for each isoform. Let this $\alpha$ value equal 50.

* 'outlier1': The first isoform of the $I$ isoforms will have a Dirichlet $\alpha$ value relatively high, resulting in a relativley high probability and the remaining probability will be approx. evenly distributed among the remaining $I-1$ isoforms ($\alpha$ values associated with these $I-1$ probabilities all constant at a lower $\alpha$ value). The high $\alpha$ value will be 50, the low $\alpha$ value will be 5.

* 'outlier2': The second isoform of the $I$ isoforms will have a Dirichlet $\alpha$ value relatively high, resulting in a relativley high probability and the remaining probability will be approx. evenly distributed among the remaining $I-1$ isoforms ($\alpha$ values associated with these $I-1$ probabilities all constant at a lower $\alpha$ value). The high $\alpha$ value will be 50, the low $\alpha$ value will be 5.

* 'outlier3': The third isoform of the $I$ isoforms will have a Dirichlet $\alpha$ value relatively high, resulting in a relativley high probability and the remaining probability will be approx. evenly distributed among the remaining $I-1$ isoforms ($\alpha$ values associated with these $I-1$ probabilities all constant at a lower $\alpha$ value). The high $\alpha$ value will be 50, the low $\alpha$ value will be 5.

All isoform Dirichlet distributions for all genes not selected for differential isoform usage will be arbitrarily assigned a 'uniform' Dirichlet distribution. For the genes selected for differential isoform usage, cell type 1 will have the outlier1 class, cell type 2 will have the outlier2 class, and cell type 3 will have the outlier3 class.

**Realistic Effect Size Option**

Genes not selected for differential isoform usage will still be assigned a 'uniform' Dirichlet distribution. However, the $\alpha$ values in the Dirichlet distributions of the genes with differential isoform usage will come from fitting transcript clusters with differential isoform usage in the Blueprint data (described in the paper). These values will be filtered such that fits resulting in `NA` $\alpha$ values or $\alpha$ values exceedingly high ($\alpha > 300) will be excluded. Full details can be found in the document "Blueprint Alpha Parameters.Rmd".

If a gene in the simulation has $I$ isoforms, then a collection of $\alpha$ values from a gene with the same number of isoforms will be selected. This collection includes the $\alpha$ values for all three cell types. These Dirichlet $\alpha$ values, specific for each cell type, will be used in the simulation process to select the $\rho_g$ parameter. 
...
For each mixture sample, there needs to be an associated fragment lengths distribution file associated with the sample in order to run the `IsoDeconvMM` fit algorithm. For the purposes of the simulation, we will simulate a single fragment length distribution file and use it for all mixture samples (we assume that the fragment length distribution is consistent across the mixtures samples).

1. Select a total read count, `total_cts`. 
...
See "Gene Samples and Effective Design Matrix.Rmd" for full details describing how this data set was used in the simulation procedure.
