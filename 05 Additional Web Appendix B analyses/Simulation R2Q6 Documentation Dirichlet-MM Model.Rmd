...
In the reviewer comments for the submitted paper, a reviewer (Referee 2) asked the following question (Q6):

"Is it possible to apply CIBERSORTx on simulation data to further support the conclusion that the method performs better when only a small group of transcript clusters are available?""

This document repeats the steps outlined in "Simulation 3 Documentation Dirichlet-MM Model.Rmd", except we have a different set-up for the differential gene expression. In the 'Simulation 3' set-up, we only set it up so that some (100) genes in CT2 had higher expression than those same genes in cell types CT1 and CT3. We will change that to the following: 33 genes will have higher expression in CT1 compared to CT2 and CT3; 33 genes will have higher expression in CT2 compared to CT1 and CT3; and 34 genes will have higher expression in CT3 compared to CT1 and CT2.


Other details of simulation:
...
## 10 to use in CIBERSORTx, 10 to use for mixture file creation
n = 20
# Number cell types
J = 3
...
## Designation of what genes are up-regulated in CT1, CT2, or CT3 compared to the other two cell types.
genes_df$CT_UpReg = c(rep("CT1",33),rep("CT2",33),rep("CT3",34))

```

```{r, eval=FALSE}
save(genes_df, file = "Simulated_Output/genes_cibersort.RData")
```
...
```{r}
# Output: New gene counts matrix where CT1, CT2, and CT3 have differentially expressed 
# compared with the other two cell types genes
#     matrix same dimension and organization of geneInfo$ct_mat
geneCtDiffExp = diff_exp_ciber(gene_counts = geneInfo$ct_mat, n = n, J = J, 
...
for(ct in 1:3){
  ct_lab = str_c("CT",ct)
  genes_up = genes_df$diffExp[which(genes_df$CT_UpReg == ct_lab)]
  mat_up = geneCtDiffExp[which(rownames(geneCtDiffExp) %in% genes_up),]
  avg_exp_ct = rowMeans(mat_up[,(1+n*(ct-1)):(n*ct)])
  ave_exp_other = rowMeans(mat_up[,-c((1+n*(ct-1)):(n*ct))])
  print(summary(avg_exp_ct/ave_exp_other))
}
```
...
```{r, eval=FALSE}
save(iso_exon_1000_3C, file = "Simulated_Output/Iso_Exon_Params_1000_Ciber.RData")
```
...
counts_output_MM(exonInfo_1000 = iso_exon_1000_3C, exonInfo_other = iso_exon_other,
                ...
                folder = "Simulated_Output/Ciber_Sim_Pure_Sample_Counts", seed = seeds[11])
