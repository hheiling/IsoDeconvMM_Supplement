...
batch_samp = rep(1:100, each = 13)[batch]
...
clust_batch = rep(1:13, times = 100)[batch]
...
prefix_results = str_c(header_nas,"Simulation_Results/Fit11A_10InitPts")
...
## load nTE_discrimE data.frame (from Blueprint_IsoDetect_Results.Rmd)
## Using an excess of 100 clusters, will select 100 later
...
# to run at one time (here, run 10 at a time). Will concatenate all p estimates later.
if(clust_batch <= 12){
  select_idx_first = (clust_batch-1)*10+1
  (select_idx = (select_idx_first):(select_idx_first+9))
}else{
  select_idx_first = (10*12+1)
...
# Select same cell type reference files as in 7A simulation
set.seed(8500)
