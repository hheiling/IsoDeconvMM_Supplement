# Use Blueprint alpha values from 11A series in Dirichlet-MM model during simulation
# In this simulation, fit using genes with differential isoform usage

# Note: Mixture files created in "01 ciber_sim_mixtures.R"
...
mix_num = 100
k = 10 # Number of batches of genes (split 100 genes into k groups)

batch_samp = rep(1:mix_num, each = k)[batch]
...
gene_batch = rep(1:k, times = mix_num)[batch]
...
prefix_results = str_c(header_nas,"deconvolution/Simulation_Results/Ref2Q6")
...
genes_idx = (1+(gene_batch-1)*k):(gene_batch*k)
genes_to_use = genesDiffUsg[genes_idx]
...
## (same as in '03 ciber_sim_pure_matrix.R' code for specifying pure samples for CIBERSORTx)
